...
cell intersects the requested bbox, and downloads the intersecting
.laz nodes into a local cache. Callers decode with laspy.

The EPT hierarchy split is a standard octree over the workunit bounds
declared in ept.json. A node key is `d-x-y-z` where d is depth and
x/y/z are cell indices within depth-d's uniform grid over root bounds.
`points` field: 0 means "no points AND no children"; >0 means "download this
node"; -1 in newer hierarchy entries means "children only, no data here".

Design point: we prefer over-fetching by half a voxel of pad to
under-fetching at the seam, so children of any node that even grazes the
bbox are considered.
"""
...
class EptRoot:
    """Just the fields we need from ept.json."""
...
def fetch_ept_root(root_url: str) -> EptRoot:
    """GET ept.json; the fields we depend on are stable across writers."""
...
def cell_bounds_3857(
    ...
    """Bounding cube of octree cell (d, x, y, z) in EPT root coords.

    The root is the cube (root_bounds), and at depth d the space is split
    into 2**d cells along each axis.
    """
...
def cell_intersects_xy(
    ...
    """Do this octree cell and the requested XY bbox overlap in X and Y?

    We ignore Z: the caller wants everything vertical inside the ground
    footprint (buildings, terrain, sky-scanned noise all live in the same
    XY column).
    """
...
def walk_intersecting_nodes(
    ...
    """Return every hierarchy key whose cell overlaps the XY bbox and has points.

    Hierarchy files are chunked. Root hierarchy at `ept-hierarchy/0-0-0-0.json`
    describes a subtree; when a node's value is negative it points to a nested
    hierarchy file at `ept-hierarchy/<key>.json`. We recurse into those.
    """
    ...
    def walk(chunk: dict[str, int]) -> None:
        for k, count in chunk.items():
            ...
            if not cell_intersects_xy(cell, xy_bbox):
                continue
            if count == 0:
                continue                              # no data, no children
            if count < 0:
                walk(load(k))                          # nested subtree
                continue
            keys.append(k)
...
def download_nodes(
    ...
    """Fetch every .laz node into cache_dir; return local paths."""
